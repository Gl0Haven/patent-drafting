...
_PATENT_MMDC_CONFIG: "Path | None" = None  # 由 --patent 设置：mmdc -c 配置（白底黑线/无阴影/中文/禁透明）


def _write_patent_mmdc_config() -> "Path":
    """写出专利附图 mermaid 配置（白底黑线、无阴影、中文字体、禁透明），返回临时文件路径。"""
    ...
    cfg = {
        ...
        "themeCSS": ".node rect,.node polygon,.node circle,.node path{filter:none !important;} .cluster rect{filter:none !important;}",
    }
    fd, name = _tf.mkstemp(suffix=".json", prefix="patent_mmdc_")
...
def _mmdc_extra_args(
    *,
    ...
    若同目录存在 puppeteer-config.json（由 setup_env.sh 生成、指向本机 chromium），自动以 -p 传入，
    规避 mmdc 自带 puppeteer 在受管环境下找不到浏览器的问题。路径由 __file__ 推导，不写死私有路径。"""
    args = [
        "-s",
        str(scale),
        "-w",
    ...
    cfg = Path(__file__).resolve().parent / "puppeteer-config.json"
    if cfg.is_file():
        args += ["-p", str(cfg)]
    if _PATENT_MMDC_CONFIG is not None:
        args += ["-c", str(_PATENT_MMDC_CONFIG)]
    return args
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--patent",
        action="store_true",
        help="专利附图模式：强制白底黑线/无阴影/中文/禁透明；隐含 --no-docx；任一围栏渲染失败即返回非零（不降级）",
    )
    p.add_argument(
        "--no-math",
        action="store_true",
    ...
    global _PATENT_MMDC_CONFIG
    if getattr(args, "patent", False):
        _PATENT_MMDC_CONFIG = _write_patent_mmdc_config()
        args.no_docx = True
    ...
    if getattr(args, "patent", False):
        if _PATENT_MMDC_CONFIG is not None:
            try:
                _PATENT_MMDC_CONFIG.unlink(missing_ok=True)
            except OSError:
                pass
        if n_fail:
            print(f"[mermaid_render] --patent：{n_fail} 处渲染失败/残留围栏，硬失败退出。", file=sys.stderr)
            return 3
