...
> **路径约定**：下列 `tools/`、`references/` 均相对本技能根目录；运行命令以 `$SKILL_DIR`（本技能安装根目录，定义见 SKILL.md「环境」节，宿主无关、脱敏不含用户名）为根。工具/知识均已并入本技能自带，无外部私有路径依赖；环境缺失走兜底并标注来源可靠性。
...
调用示例：
```bash
conda run --no-capture-output -n patent-drafting python "$SKILL_DIR/tools/cnipa_epub_search.py" "语义词块"
```
...
```bash
conda run --no-capture-output -n patent-drafting python "$SKILL_DIR/tools/case_matcher.py" --features "特征1,特征2,特征3" --format text
```
