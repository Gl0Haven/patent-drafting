...
2. **SC-2 读码梳理**：Glob/Grep 识别技术栈与入口，定位界面定义/控件/事件回调精读（四栈启发式见参考文档 §4），其余采样，产出中间稿（界面区块树 / 逐步操作流程，或无 GUI 时模块-函数-数据流）。**只写代码里有的**。操作流程不直接读码成文，走 **§4.5「抽取(读码+挖 E2E测试/文档/菜单/示例) → 功能-顺序确认表 → 逐功能确认(粒度 A/B 由用户选) → 装配」**，把顺序与主次锚定到作者，主线与"其他功能"分开。
...
- 软著（§SC）：`tools/sc_source_doc.py`（扫码→裁剪前30后30、≥50行/页、去空行、页眉=软件名+版本、右上页码→出"源代码.docx"，并实测总行数回填申请表；`--src` 可多次指定代码文件夹，`--dry-run` 仅自检）、`tools/sc_ui_capture.py`（Web/Electron 走 playwright 自动截图，失败即降级退占位）。说明书与字段表用 `md_to_docx.py`/`mermaid_render.py` 出件，规范见 `references/software-copyright-writing.md`。
